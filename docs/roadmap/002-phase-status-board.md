...
⁵ Phase 9 sai de `SPEC_DRAFTED` para `IMPLEMENTING` apos o Slice 9-O1 (2026-07-01). Entrega da slice 9-O1: header HTTP `X-Correlation-Id` inbound/outbound, coluna `correlation_id VARCHAR(64) NULL` em `webhook_events` + `outbox_events` (migration `20260701000001_AddObservabilityColumns`), `CorrelationIdMiddleware` registrado ANTES de `ApiKeyAuthenticationMiddleware` em `Program.cs`, 13 counters + 3 histograms no `Meter` "PaymentHub" com tag whitelist (7 chaves), catalogo `PaymentHubLogEvents` com 31 eventos canonicos, helpers `SafeLog` (`Id`/`Length`/`Flag`/`Category`), gate regex anti-leak em `scripts/agent-docs-check.sh` cobrindo 6 tokens sensitive, suite 547 testes (489 baseline medido em HEAD~1 + 58 novos em 9-O1). Worker host continua sem `HttpContext` (usa `NullCorrelationIdAccessor` Singleton). **Slice 9-O1.IT (2026-07-01) fecha o gap E2E deixado pela 9-O1**: 4 testes em `tests/PaymentHub.IntegrationTests/EndToEnd/CorrelationIdE2ETests.cs` via `WebApplicationFactory<Program>` + Testcontainers Postgres. Endpoint `GET /health` (anonymous, retorna 404 — middleware corre antes do routing). Total integration agora 37 testes (33 baseline + 4 novos). **Bug critico corrigido**: migration `20260701000001_AddObservabilityColumns` shipped em commit `aaa9ea5` SEM o correspondente `.Designer.cs`; sem o Designer, EF Core `MigrateAsync()` contra Postgres real nao aplicava a coluna `correlation_id`. Hand-author do Designer.cs copiando template `20260630184619` + 2 property mappings. Suite final **584 testes (547 unit + 37 integration)**. **Slice 9-O2 (2026-07-02) instrumenta 8 call sites**: `CreateCheckoutHandler`, `ApiKeyAuthenticationMiddleware`, `AbacatePayClient`, `AbacatePayWebhookManagementClient`, `ProviderWebhooksController`, `ProcessWebhookEventHandler`, `OutboxDispatcherWorker`, `HttpApplicationWebhookDispatcher`. 4 novos instrumentos adicionados ao catalogo: `CheckoutFailedTotal`, `ProviderCallTotal`, `ProviderCallFailedTotal`, `ProviderCallDurationMs` (16 counters + 4 histograms no total). Padrao `Stopwatch.GetTimestamp()` + try/catch/finally + `Record()` extension method (bridge `TagList` → `KeyValuePair[]` para .NET 10 Counter.Add). Anti-leak preservado: tag whitelist 7 chaves, SafeLog helpers (`Id`/`Length`/`Category`), sem `apiKey`/`webhookSecret`/`rawPayload`/`signature`/`Authorization`/`body` em logs. Suite final **594 testes (557 unit + 37 integration)** sem regressao. Restam para 9-O3+: distributed tracing via `Activity`/OpenTelemetry (fora de escopo MVP) + dashboard Grafana (backlog). Detalhes em `docs/audits/slice-9-o1-observability-minimal-report-2026-07-01.md`, `docs/audits/slice-9-o1-it-correlationid-e2e-report-2026-07-01.md` e `docs/audits/slice-9-o2-active-instrumentation-report-2026-07-02.md`.

...
| PH-PROVIDER-WEBHOOK-RAWPAYLOAD-TEXT | Banco | Migrar `webhook_events.raw_payload` de `jsonb` para `text` (HMAC byte preservation) | Concluido | `docs/specs/010-database-contract.md` | Slice 3-IT (2026-06-29) — bug encontrado por E2E testing. Postgres `jsonb` reformata JSON no insert (espacos, normaliza chaves), o que quebra HMAC sobre o body bruto. Migracao `20260629205545_ChangeRawPayloadToText` aplicada. |
| PH-PROVIDER-WEBHOOK-ATTEMPT-TRACKING | Provider | Forcar `Add` explicito do novo `PaymentAttempt` no `ProcessWebhookEventHandler` | Concluido | `docs/specs/006-provider-webhooks.md` | Slice 3-IT (2026-06-29) — bug encontrado por E2E testing. Collection navigation tracking do EF Core 10 nao detecta confiavelmente itens adicionados via `entity.Navigation.Add(item)`; o item aparecia como `Modified` em vez de `Added`, levantando `DbUpdateConcurrencyException`. Solucao: chamar `_payments.AddAttemptAsync(attempt, ct)` apos `payment.RegisterAttempt(...)`. |
